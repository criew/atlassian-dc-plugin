...
JIRA_SCRIPTS = SKILLS_DIR / "jira-dc" / "scripts"
BB_SCRIPTS = SKILLS_DIR / "bitbucket-dc" / "scripts"
CONF_SCRIPTS = SKILLS_DIR / "confluence-dc" / "scripts"

for _d in (JIRA_SCRIPTS, BB_SCRIPTS, CONF_SCRIPTS):
...
def _resolve_script_path(relpath: str) -> Path:
    ...
    if name.startswith("confluence_"):
        return CONF_SCRIPTS / relpath
    if name.startswith("bitbucket_"):
        return BB_SCRIPTS / relpath
    return JIRA_SCRIPTS / relpath
